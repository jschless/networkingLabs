...
- [x] **5.2 `labs/ztp-basics`** — day-0 provisioning: factory-default cEOS boots ZTP,
      DHCP option 67 → HTTP config server; break-it: wrong bootfile/unreachable server.
- [x] **5.3 `labs/anycast-dns`** — routing-on-the-host anycast service: two DNS
...
- [x] **5.4 `labs/k8s-fabric`** — Kubernetes ↔ fabric integration: k3s/kind node(s)
...
- [x] **5.5 `labs/service-ha`** — stateful service HA the way enterprises actually run
...
|------|------|----------------|
| 2026-07-05 | Phase 5 MERGED | PR #32 (5.2 ztp-basics, 5.3 anycast-dns, 5.4 k8s-fabric, 5.5 service-ha) merged to main (commit 68fd772); all four lab dirs verified present on origin/main (no merge race). 5.1 mpls-ldp was PR #31. Phase 5 (5.1–5.5) complete. CI fix folded in: mkdocs `--strict` rejects markdown links to `../<lab>/README.md` from an included lab README — reference sibling labs by name in backticks (repo convention). |
| 2026-07-05 | 5.5 service-ha — PHASE 5 BUILD COMPLETE | Built + fully validated live on branch `lab/ztp-basics` (shared PR #32, the last of 5.2–5.5). Stateful firewall HA: keepalived VIP pair (vrrp_sync_group, both VIPs atomic) + conntrackd FTFW state sync over a dedicated link; nftables stateful forward chain (`ct state invalid drop`) with `nf_conntrack_tcp_loose=0`. **The lesson, validated by iperf3 --forceflush trace:** a long-lived TCP flow **dies** on failover WITHOUT conntrackd (post-failover 0.00 bits/sec forever — mid-stream packets are INVALID w/ tcp_loose=0) and **survives** WITH it (~2s VRRP dip then full recovery; fw2's conntrackd -e external cache holds the flow pre-failover, notify_master→`conntrackd -c` commits it on takeover). check.sh 15/15 incl. an automated failover-survival probe, on a clean redeploy. **Prototype-first confirmed the kernel supports it:** linuxkit has `nf_conntrack_events=2` + timestamp/acct, tcp_loose writable per-netns, conntrackd multicast sync works. **Traps:** debian-slim lacks `/etc/protocols` → conntrackd TCP filter fails (added `netbase`); `pkill keepalived` is NOT a real failover (container keeps forwarding) — use `docker stop`/`ip link down` on the active; zombie keepalived if restarted sloppily; iperf3 output is block-buffered to files → need `--forceflush` to see the failover live. New image `service-ha:local` (debian+keepalived/conntrackd/nftables). Registered in HA track. **All of Phase 5 (5.1–5.5) is now built; PR #32 (5.2–5.5) is ready to merge.** |
