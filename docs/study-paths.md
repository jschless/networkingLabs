...
| **Cisco NHRP/DMVPN syntax** (`ip nhrp map`, `tunnel mode gre multipoint`) | VyOS DMVPN (`dmvpn-phase1/2/3`) — Phase 1 mGRE/NHRP mechanics transfer with different config grammar; `dmvpn-phase2` is a current-image compatibility reference whose direct path needs Traffic-Indication/shortcut optimization rather than classic Phase 2 ordinary resolution; `dmvpn-phase3` uses overlay-only OSPF plus a hub-owned external service summary and observes current-image service-host mappings with service-prefix `/24` shortcuts |
