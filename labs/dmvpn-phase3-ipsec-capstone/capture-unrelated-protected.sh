#!/usr/bin/env bash
# Prove the unrelated spoke2-to-spoke3 shortcut remains encrypted.
set -euo pipefail

usage() {
    printf '%s\n' 'Usage: labs/dmvpn-phase3-ipsec-capstone/capture-unrelated-protected.sh'
}
case ${1:-} in -h|--help) usage; exit 0 ;; '') ;; *) usage >&2; exit 2 ;; esac
(( $# == 0 )) || { usage >&2; exit 2; }

prefix=clab-dmvpn-phase3-ipsec-capstone
capture=$(mktemp -t dmvpn-capstone-unrelated.XXXXXX)
capture_pid=
capture_filter=
...
finish_capture() {
    ...
    if ! wait_filter_absent "$filter"; then
        echo 'ERROR: unrelated-path capture left its exact tcpdump process active' >&2
        return 1
    fi
    if (( status == 0 )); then return 0; fi
    echo "ERROR: unrelated-path capture exited with status $status" >&2
    return 1
}
stop_capture() {
...
for _attempt in $(seq 1 30); do
    timeout 5 docker exec "$prefix-spoke2" ping -I 192.168.2.1 -c 1 -W 1 \
        192.168.3.1 >/dev/null 2>&1 || true
    fib=$(docker exec "$prefix-spoke2" ip -4 route get 192.168.3.1 \
        from 192.168.2.1 2>/dev/null || true)
    grep -qE '^192\.168\.3\.1 from 192\.168\.2\.1 (via 172\.16\.0\.13 )?dev tun0([[:space:]]|$)' <<<"$fib" && break
    sleep 1
done
unrelated_filter='host 10.0.0.12 and host 10.0.0.13 and (ip proto 47 or ip proto 50)'
start_capture "$capture" 7 "$unrelated_filter"
sleep 1
timeout 6 docker exec "$prefix-spoke2" ping -I 192.168.2.1 -c 3 -W 1 \
    192.168.3.1 >/dev/null
finish_capture
grep -qE '10\.0\.0\.12 > 10\.0\.0\.13: ESP' "$capture"
grep -qE '10\.0\.0\.13 > 10\.0\.0\.12: ESP' "$capture"
if grep -qE '10\.0\.0\.(12|13) > 10\.0\.0\.(12|13): GRE' "$capture"; then
    echo 'ERROR: raw GRE leaked on the unrelated protected path' >&2
    exit 1
fi
echo 'PASS: the unrelated spoke2-to-spoke3 shortcut remains bidirectionally protected.'
