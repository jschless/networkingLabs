...
node_id="$(curl -s -u "${auth}" "${base_url}/nodes?label=campus1" | sed -n 's/.*<node[^>]*id="\([0-9]\+\)".*label="campus1".*/\1/p' | head -n1)"

if [[ -z "${node_id}" ]]; then
    echo "campus1 is not present in OpenNMS yet." >&2
    exit 1
fi

echo "campus1 node id: ${node_id}"
echo
echo "IP interfaces:"
