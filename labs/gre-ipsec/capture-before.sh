#!/usr/bin/env bash
# Generate and validate a bounded clear-text GRE observation for Task 1.
set -euo pipefail

usage() {
    printf '%s\n' 'Usage: labs/gre-ipsec/capture-before.sh' '' \
        'Capture the pre-IPsec host flow on the transit link and require readable GRE/ICMP evidence.'
}
...
capture_file=$(mktemp -t gre-ipsec-before.XXXXXX)
capture_pid=
cleanup() {
...
if grep -qE '203\.0\.113\.1 > 203\.0\.113\.6: GREv0' "$capture_file" \
    ...
    echo "PASS: the bounded WAN capture exposes raw GRE and readable private ICMP."
    exit 0
fi

echo "ERROR: the capture did not prove the required clear-text GRE flow" >&2
exit 1
