...
cp /opt/lab-pki/golden/corp-user.pem /etc/wpa_supplicant/client.pem
cp /opt/lab-pki/golden/corp-user.key /etc/wpa_supplicant/client.key
cat >/etc/wpa_supplicant/corp.conf <<'EOF'
network={
  key_mgmt=IEEE8021X
  eap=TLS
  identity="corp-user"
