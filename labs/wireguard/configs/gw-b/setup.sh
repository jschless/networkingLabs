...
ip addr add 10.0.0.20/24 dev eth1 2>/dev/null || true
ip link set eth1 up
mkdir -p /etc/wireguard
chmod 700 /etc/wireguard

echo "gw-b ready."
echo "  WAN: 10.0.0.20/24 on eth1"
echo "  Hub WAN: 10.0.0.1"
echo ""
echo "Steps to configure WireGuard:"
echo "  1. wg genkey | tee /etc/wireguard/gwb.key | wg pubkey > /etc/wireguard/gwb.pub"
