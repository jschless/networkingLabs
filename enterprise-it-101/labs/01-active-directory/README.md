...
??? note "Hints"
    - `ldapsearch -x` does an anonymous (simple) bind; `-H ldap://10.100.1.10` sets the server, `-b "DC=lab,DC=corp"` the search base.
    - Try an *authenticated simple bind* too — `-D "alice@lab.corp" -w 'P@ssw0rd1'` — and read the error carefully. AD refuses to accept a password over an unencrypted connection.
    - The way to authenticate **without** a TLS certificate (you don't get one until Lab 03) is Kerberos: `kinit alice@LAB.CORP` first, then bind with `-Y GSSAPI` (no `-D`/`-w`). GSSAPI signs and seals the connection, which satisfies AD's "strong auth" requirement.
    - Request specific attributes by listing them after the filter: `... sAMAccountName memberOf unicodePwd`.
...
??? note "Solution"
    ...
    # 2) Authenticated *simple* bind over plaintext ldap:// — watch it get refused.
    ldapsearch -x -H ldap://10.100.1.10 -b "DC=lab,DC=corp" \
        -D "alice@lab.corp" -w 'P@ssw0rd1' \
        "(sAMAccountName=alice)" sAMAccountName memberOf

    # 3) Authenticate with Kerberos instead, then bind with GSSAPI (signed+sealed).
    kinit alice@LAB.CORP                 # enter P@ssw0rd1
    ldapsearch -Y GSSAPI -H ldap://dc1.lab.corp -b "DC=lab,DC=corp" \
        "(sAMAccountName=alice)" sAMAccountName memberOf

    # 4) As alice, try to read a password attribute — note what comes back.
    ldapsearch -Y GSSAPI -H ldap://dc1.lab.corp -b "DC=lab,DC=corp" \
        "(sAMAccountName=bob)" unicodePwd
    ```

??? success "Check your work"
    - **Anonymous** binds against AD return little or nothing — you'll likely see `result: 1 Operations error`. Modern AD disables anonymous reads of user objects by default: a stranger shouldn't be able to enumerate your directory.
    - **The authenticated *simple* bind is rejected** with `ldap_bind: Strong(er) authentication required ... Transport encryption required`. This is the lesson: AD will not accept your password over a cleartext connection. You must either use TLS (LDAPS — that's Lab 03) or a SASL mechanism that protects the channel.
    - **The GSSAPI bind succeeds** (`SASL/GSSAPI authentication started` → `result: 0 Success`) and returns alice's `memberOf` — `engineering` and `all-staff`. Kerberos authenticated you *and* encrypted the session, with no certificate involved. This is why a domain-joined machine can query LDAP securely the moment it has a ticket.
    - **No one** gets `unicodePwd` back — the query succeeds but the attribute simply isn't returned, even to an authenticated user, even for your own account. Authorization in AD is per-attribute, not just per-object. "I can see the user exists" and "I can read their secrets" are completely different privileges.
...
# Grammar: samba-tool dns delete <server> <zone> <name> <type> <data>
# The <name> is the record relative to the zone (_kerberos._tcp), NOT the FQDN,
# and the SRV <data> order is: "target port priority weight"
samba-tool dns delete dc1.lab.corp lab.corp _kerberos._tcp SRV "dc1.lab.corp 88 0 100" -U Administrator
...
# LDAP — authenticated (GSSAPI) search returns alice with her groups.
# Uses the TGT from the kinit above; AD refuses a cleartext simple bind.
ldapsearch -Y GSSAPI -H ldap://dc1.lab.corp -b "DC=lab,DC=corp" \
